...
class Extension:
    ...
    def main(self, window, side=None, amount=None):
        mark("resize start", source="resize")
        log(f"requested resize side={side} amount={amount}", important=False, source="resize")
        check_types(
            side=(side, str),
        ...
        try:
            if window is None:
                log("no window supplied; prompting for title", source="resize")
                title = input("Window title: ")
                matches = windows.find_by_title(title)
                if not matches:
                    print("No matching window found.")
                    log(f"no matching window found for title fragment={title}", source="resize")
                    return
                window = matches[0]
            log(f"resizing window={window.title}", source="resize")

            if not side:
                print("Missing required argument: side")
                log("resize aborted because side argument was missing", source="resize")
                return window
            ...
            if direction is None:
                ...
                log(f"resize direction could not be resolved from side={side}", source="resize")
                return window

            if amount is not None:
                log(f"running one-off resize direction={direction} delta={amount}", source="resize")
                self._resize_once(window, direction, int(amount))
            else:
                print(f"Interactive resize mode ({direction}). Use arrow keys, ESC to exit.")
                log(f"starting interactive resize direction={direction}", source="resize")
                self._interactive_resize(window, direction)

        except pywintypes.error:
            print("Window is no longer valid (likely closed).")
            log("window handle became invalid during resize", source="resize")
        except Exception as e:
            print(f"An error occurred: {e}")
            log(f"resize failed: {e}", source="resize")

        mark("resize complete", source="resize")
        return window

    def _resize_once(self, window, direction, delta):
        log(f"_resize_once start direction={direction} delta={delta}", important=False, source="resize")
        ...
        window.resizeTo(w, h)
        log(f"_resize_once applied left={l} top={t} width={w} height={h}", source="resize")
    ...
    def _interactive_resize(self, window, direction):
        ...
        log(f"interactive resize config amount={resize_amount} duration={duration} steps={steps}", source="resize")
        ...
        def anim_loop():
            while not target["stop"]:
                ...
                log(f"interactive resize target={target}", important=False, source="resize")
                time.sleep(0.01)
        ...
        def handle_input():
            while True:
                if keyboard.is_pressed("esc"):
                    ...
                    log("interactive resize stopped by esc", source="resize")
                    break
